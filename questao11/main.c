...
void Get_input(int my_rank, int comm_sz, double* a_p, double* b_p, int* n_p) {
    int position = 0;
    int bufsize = 2 * sizeof(double) + sizeof(int);
    char buffer[bufsize];
    ...
    if (my_rank == 0) {
        ...
        MPI_Pack(a_p, 1, MPI_DOUBLE, buffer, bufsize, &position, MPI_COMM_WORLD);
        MPI_Pack(b_p, 1, MPI_DOUBLE, buffer, bufsize, &position, MPI_COMM_WORLD);
        MPI_Pack(n_p, 1, MPI_INT, buffer, bufsize, &position, MPI_COMM_WORLD);
    }
    ...
    if (my_rank != 0) {
        position = 0;
        MPI_Unpack(buffer, bufsize, &position, a_p, 1, MPI_DOUBLE, MPI_COMM_WORLD);
        MPI_Unpack(buffer, bufsize, &position, b_p, 1, MPI_DOUBLE, MPI_COMM_WORLD);
        MPI_Unpack(buffer, bufsize, &position, n_p, 1, MPI_INT, MPI_COMM_WORLD);
    }
}
